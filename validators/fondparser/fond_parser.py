...
from .fond_task import FONDTask
...
class Parser:
  ...
  def generate_file(self, sas_file_name):
    if self.domain == None or self.problem == None:
      raise Error('Domain and/or problem not set!')
    time_limit = 1000
  ...
  def generate_task(self, sas_file_name):
    try:
      ...
      raise Error('Error opening sas file!')
  ...
  def __process_effects(self, lines, operator):
    for line in lines:
      l_split = line.split(' ')
      if len(l_split) != 4:
        raise Error('Incorrect number of components in effects of an operator!')
      if l_split[0] != '0':
        raise Error('First component of effects != 0!')
      var = int(l_split[1])
      pre = int(l_split[2])
      eff = int(l_split[3])
  ...
  def __process_prevail_conditions(self, lines, operator):
    for line in lines:
      try:
        ...
        raise Error('Error processing prevail conditions!')
  ...
  def __get_limits(self, lines):
    ...
    if len(initis) != len(ends):
      raise Error('Inits and ends of different size!')
