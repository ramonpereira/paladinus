...
/**
 * Partial {@link OptionHandler} implementation that takes a single value to the
 * option, which is then gets split into individual tokens using fixed
 * delimiter.
 *
 * <p>
 * This class is marked as {@code abstract} even though it has no abstract
 * methods to indicate that the class cannot be used by itself in
 * {@link Option#handler()}, due to the extra argument that it takes.
 *
 * @author kmahoney
 */
public abstract class DelimitedOptionHandler<T> extends OptionHandler<T> {
	...
	public DelimitedOptionHandler(CmdLineParser parser, OptionDef option, Setter<? super T> setter, String delimiter,
