 ...
 *
 * <p>
 * This class can be extended by application to support additional Java
 * datatypes in option operands.
 *
 * <p>
 * Implementation of this class needs to be registered to args4j by using
 * {@link CmdLineParser#registerHandler(Class,Class)}
 *
 * @param <T> The {@code component} type of the field that this
 *            {@link OptionHandler} works with. When I say "component", I mean a
 *            field that can hold multiple values (such as {@link Collection} or
 *            array). This should refer to its component time.
 *
 *            {@link Setter} implementations abstract away multi-value-ness by
 *            allowing {@link OptionHandler} to invoke its
 *            {@link Setter#addValue(Object)} multiple times.
 *
...
public abstract class OptionHandler<T> {
	...
	protected OptionHandler(CmdLineParser parser, OptionDef option, Setter<? super T> setter) {
		// myND Adaption: set default value
		if (setter.asFieldSetter() != null) {
			option.setDefault(setter.asFieldSetter().getValue());
		}
		this.owner = parser;
		this.option = option;
		this.setter = setter;
	 ...
	 *
	 * @param params The rest of the arguments. This method can use this object to
	 *               access the arguments of the option if necessary.
	 *
	 *               The object is valid only during the method call.
	 *
	 * @return The number of arguments consumed. (For example, returns {@code 0} if
	 ...
	 * {@code ResourceBundle}, if one was passed to {@link CmdLineParser}.
	 *
	 ...
	 * "name=metaval, e.g. "--foo VALUE" or "--foo=VALUE"
	 *
	 * @param rb         ResourceBundle to get localized version of meta string
	 * @param properties Affects the formatting behaviours.
	 */
