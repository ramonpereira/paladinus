 ...
 *
 * <p>
 * With this, <samp>-P x=1 -P y=2</samp> parses to map of size {@code 2}. This
 * option handler can be subtyped if you want to convert values to different
 * types or to handle <samp>key=value</samp> in other formats, like
 * <samp>key:=value</samp>.
 */
	...
	protected void addToMap(String argument, Map m) throws CmdLineException {
		...
		if (idx >= 0) {
			...
			if (mapValue.length() == 0) {
				// Kohsuke: I think this is a bad choice, but this is needed to remain backward
				// compatible
				mapValue = null;
			}
		} else {
