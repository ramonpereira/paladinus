 ...
 *
 * <p>
 * For example, we use this to let the caller specify which options are printed,
...
public interface OptionHandlerFilter {
	/**
	 *
	 * @param o Never {@code null}. Internally options (like <samp>-r</samp>) and
	 *          arguments (others) are treated uniformly as {@link OptionHandler}.
	...
	/**
	 * myND adaption: Print all hidden options.
	 */
	OptionHandlerFilter HIDDEN = new OptionHandlerFilter() {
		@Override
		public boolean select(OptionHandler o) {
			return o.option.hidden();
		}
