...
/**
 * {@link OptionHandler} used with {@link Argument} for parsing typical
 * "sub-command" pattern.
 *
 * <p>
 * The "sub-command" pattern refers to the design of the command line like
 * <tt>git</tt> and <tt>svn</tt>, where the first argument to the command
 * designates a sub-command (say <samp>git checkout</samp>), then everything
 * that follows afterward are parsed by this sub-command (which is usually
 * different depending on which sub-command was selected.)
 *
 * <p>
 * This {@link OptionHandler} models this design pattern with the
 * {@link SubCommands} annotation. See the following example:
 *
 * <code><pre>
 * class Git {
 ...
 *
 * <p>
 * An example of legal command line option for this is <samp>-r checkout
 * -a</samp>.
 *
 * <ul>
 * <li>{@link SubCommand} only works with {@link Argument} and not with
 * {@link Option}.
 *
 * <li>The same field/setter must be also annotated with {@link SubCommands}
 * that specify possible sub-commands.
 *
 * <li>Any {@link Option}s that you define in the <tt>Git</tt> class above can
 * parse options that appear prior to the sub-command name. This is useful for
 * defining global options that work across sub-commands.
 *
 * <li>The matching sub-command implementation gets instantiated with the
 * default constructor, then a new {@link CmdLineParser} will be created to
 * parse its annotations.
 *
 * <li>The rest of the arguments that follow the sub-command will be parsed with
 * this new {@link CmdLineParser}
 *
 * <li>The fully populated sub-command instance is set as the value.
 * </ul>
 *
 * <p>
 * This class defines a number of protected methods that allow subtypes to
 * customize various parts of the behaviours. This should also serve as an
 * example if you want to combine this with more sophisticated sub-command
 * lookup, such as through META-INF/services,
 * <a href="http://sezpoz.java.net/">sezpoz</a>, or
 * <a href="https://github.com/jenkinsci/lib-annotation-indexer">annotation
 * indexer</a>.
 *
